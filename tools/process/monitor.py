...
def get_system_info() -> dict:
    ...
    try:
        if subprocess.run(["which", "free"], capture_output=True).returncode == 0:
            # Linux
            result = subprocess.run(
                ["free", "-h"],
                capture_output=True,
        ...
        info["memory"] = f"Could not retrieve memory info: {str(e)}"
    ...
    try:
        ...
        info["disk"] = f"Could not retrieve disk info: {str(e)}"
    ...
    try:
        ...
        info["uptime"] = f"Could not retrieve uptime: {str(e)}"
