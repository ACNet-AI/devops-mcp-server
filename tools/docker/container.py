...
def docker_stop(container: str, timeout: int = 10) -> dict:
    ...
    try:
        ...
        return {
            ...
            "container": container,
            "message": f"Container {container} stopped successfully" if result.returncode == 0 else f"Failed to stop container {container}",
            "output": result.stdout,
...
def docker_start(container: str) -> dict:
    ...
    try:
        ...
        return {
            "success": result.returncode == 0,
            "container": container,
            "message": f"Container {container} started successfully" if result.returncode == 0 else f"Failed to start container {container}",
            "output": result.stdout,
            "error": result.stderr if result.returncode != 0 else None
        }
    except FileNotFoundError:
...
def docker_restart(container: str, timeout: int = 10) -> dict:
    ...
    try:
        ...
        return {
            "success": result.returncode == 0,
            "container": container,
            "message": f"Container {container} restarted successfully" if result.returncode == 0 else f"Failed to restart container {container}",
            "output": result.stdout,
            "error": result.stderr if result.returncode != 0 else None
        }
    except FileNotFoundError:
